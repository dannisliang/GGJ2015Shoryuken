...
public static class DTGUIHelper {
	...
	private const string DB_TEXT = " (dB)";
	private const float LED_FRAME_TIME = .07f;
	private const float MIN_DB = -140;
    ...
    public static DTFunctionButtons AddDynamicGroupButtons(UnityEngine.Object obj) {
        GUIContent deleteIcon = null;
        GUIContent settingsIcon;
        GUIContent previewIcon = null;
	...
	public static float DisplayVolumeField(float vol, VolumeFieldType fieldType, bool wideMode, float volumeMin = 0f, bool showFieldName = false, string fieldName = "Volume") {
		...
		var dbLevel = (float) System.Math.Round(AudioUtil.GetDbFromFloatVolume(vol), 1);

		var newDbLevel = 0f;
		switch (fieldType) {
			case VolumeFieldType.MixerGroup:
                newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(wideMode ? WideModeWidth : NormalModeWidth));
				break;
			case VolumeFieldType.DynamicMixerGroup:
				newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(100));
				break;
			case VolumeFieldType.Bus:
				newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(wideMode ? WideModeWidth : NormalModeWidth));
				break;
			case VolumeFieldType.PlaylistController:
				newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(74));
				break;
			case VolumeFieldType.None:
				if (showFieldName) {
					newDbLevel = EditorGUILayout.Slider(fieldName + DB_TEXT, dbLevel, MIN_DB, MAX_DB);
				} else {
					newDbLevel = EditorGUILayout.Slider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(252));
				}
				break;
		}
    ...
    public static void AddLedSignalLight(MasterAudio sounds, string groupName) {
        ...
        if (Application.isPlaying) {
			...
			if (groupInfo != null && !groupInfo._playedForWarming && groupInfo._lastTimePlayed > 0f && groupInfo._lastTimePlayed <= Time.realtimeSinceStartup) {
    ...
    public static DTFunctionButtons AddFoldOutListItemButtons(int position, int totalPositions, string itemName, bool showAfterText, bool showMoveButtons = false, bool showAudioPreview = false) {
        ...
        // Remove Button - Process presses later
        bool removePressed = GUILayout.Button(new GUIContent(MasterAudioInspectorResources.deleteTexture, "Click to remove " + itemName),
                                                  EditorStyles.toolbarButton);
    ...
    private static PrefabType GetPrefabType(UnityEngine.Object gObject) {
        return PrefabUtility.GetPrefabType(gObject);
    }

	public static bool IsPrefabInProjectView(UnityEngine.Object gObject) {

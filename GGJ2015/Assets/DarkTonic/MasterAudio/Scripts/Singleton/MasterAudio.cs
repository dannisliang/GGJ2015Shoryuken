...
public class MasterAudio : MonoBehaviour {
	...
	public const string AudioSourceTemplateFolder = MasterAudio.MasterAudioFolderPath + "/Sources/Prefabs/AudioSourceTemplates/";
    ...
    private static PlaySoundResult PlaySoundIfAvailable(string soundGroupName,
        ...
        float calcVolume = varVol * groupVolume * busVolume * MasterAudio.Instance._masterAudioVolume;
    ...
    public static void SetGroupVolume(string sType, float volumeLevel) {
        ...
        for (var i = 0; i < theGroup._sources.Count; i++) {
            ...
            if (aInfo.variation.randomVolumeMode == SoundGroupVariation.RandomVolumeMode.AddToClipVolume) {
                newVol = (aInfo.originalVolume * aInfo.lastPercentageVolume * aGroup.groupMasterVolume * busVolume * MasterAudio.Instance._masterAudioVolume) + aInfo.lastRandomVolume;
            } else { // ignore original volume
                newVol = (aInfo.originalVolume * aInfo.lastPercentageVolume * aGroup.groupMasterVolume * busVolume * MasterAudio.Instance._masterAudioVolume);
            }
    ...
    private static void SetBusVolume(GroupBus bus, float newVolume) {
        ...
        foreach (var key in audioSourcesBySoundType.Keys) {
            ...
            for (var i = 0; i < aGroup._sources.Count; i++) {
				...
				var grpVol = aGroup._group.groupMasterVolume * bus.volume * MasterAudio.Instance._masterAudioVolume;
    ...
    public static float PlaylistMasterVolume {
        get {
            return MasterAudio.Instance._masterPlaylistVolume;
        }
        set {
            MasterAudio.Instance._masterPlaylistVolume = value;
    ...
    public static float MasterVolumeLevel {
        get {
            return MasterAudio.Instance._masterAudioVolume;
        }
        set {
            MasterAudio.Instance._masterAudioVolume = value;
